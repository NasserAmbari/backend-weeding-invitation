...
export class RsvpService {
    static async createRsvp(data) {
        if (!data.fullname || !data.message || !data.attendance) {
            throw new Error("All fields (fullname, message, attendance) are required.");
        }
        const validAttendances = ["Hadir", "Tidak Hadir", "Tentative"];
        if (!validAttendances.includes(data.attendance)) {
            throw new Error("Invalid attendance value. Must be 'Hadir', 'Tidak Hadir', or 'Tentative'.");
        }
        const emoji = data.attendance === "Hadir"
            ? "✅"
            : data.attendance === "Tidak Hadir"
                ? "❌"
                : "🤔";
        await sendTelegramMessage(`RSVP Baru!\n\n` +
            `👤 Nama: ${data.fullname}\n` +
